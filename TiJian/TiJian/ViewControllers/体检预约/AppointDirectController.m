...
#import "AppointListViewController.h" //未预约列表(公司或者个人)
 ...
 *  @param sender
 */
- (void)pushToMoreList:(UIButton *)sender
{
    AppointListViewController *list = [[AppointListViewController alloc]init];
    list.listType = (sender.tag - 100) == 0 ? ListType_Company : ListType_Normal;
    list.myTitle = (sender.tag - 100) == 0 ? _companyName : @"个人套餐";
...
- (UIView *)viewForHeaderInSection:(NSInteger)section tableView:(UITableView *)tableView
{
    ...
    {
        //箭头
        UIImageView *arrow = [[UIImageView alloc]initWithFrame:CGRectMake(DEVICE_WIDTH - 35 + 5, 0, 35, 40)];
        arrow.image = [UIImage imageNamed:@"personal_jiantou_r"];
        arrow.contentMode = UIViewContentModeCenter;
        [head addSubview:arrow];
        
        [head addTaget:self action:@selector(pushToMoreList:) tag:100 + section];
    }
