...
//        self.backgroundColor = [UIColor whiteColor];
        ...
        datePicker = [[UIDatePicker alloc] initWithFrame:CGRectMake(0, 44, DEVICE_WIDTH, 216)];
...
- (void)clickDoneButton:(UIButton *)sender
{
    if (sender.tag == 100) {
        ...
        if (dateBlock) {
            dateBlock(ACTIONTYPE_SURE, [self selectTime:datePicker.date]);
        }
    }else if (sender.tag == 101){
        
        if (dateBlock) {
            dateBlock(ACTIONTYPE_CANCEL, @"取消");
        }
    }
...
- (void)updatetime:(UIDatePicker *)sender
{
    ...
    if (dateBlock) {
        dateBlock(ACTIONTYPE_NORMAL,[self selectTime:sender.date]);
    }
}
